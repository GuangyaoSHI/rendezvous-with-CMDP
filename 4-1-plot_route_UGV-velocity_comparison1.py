...
experiment_name = '_velocity_comparison1'
threshold=0.1
data_name_appendix = '-1'
    ...
    if type(UAV_traces[i]) == int:
        if type(UAV_traces[i+1]) == int:
            ...
            for j in range(len(x)):
                axs.text(x[j], y[j], route_labels[j], fontsize = 'small')   
        else:
            ...
            for rl in route_labels:
                axs.text(rl[0], rl[1], route_labels[rl], fontsize = 'small')
