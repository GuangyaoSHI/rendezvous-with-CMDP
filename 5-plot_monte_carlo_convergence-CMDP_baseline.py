...
if __name__ == "__main__":
    ...
    x=3
    axs1.bar(x-width-width, 1-prob_success[threshold], width, color='r')
    axs1.bar(x-width, max(1-success_baseline_prob[40], 0.02), width, color='g')
    axs1.bar(x, max(1-success_baseline_prob[50], 0.02), width, color='b')
    axs1.bar(x+width, max(1-success_baseline_prob[60], 0.02), width, color='m')
    axs1.bar(x+width+width, max(1-success_baseline_prob[70], 0.02), width, color='orange')
    axs1.set_xticks([0, 1.5, 3], ['task duration', 'success \n task duration', "failure probability"])
    axs1.set_ylabel("Probability")
    axs1.set_ylim(0, 1)
    x_tour = np.arange(-0.5, 2.1, 0.1)
    y_tour = tour_dis*np.ones((len(x_tour), ))/rendezvous.velocity_uav['v_br']
    axs.plot(x_tour, y_tour, color='k', linestyle='--')
